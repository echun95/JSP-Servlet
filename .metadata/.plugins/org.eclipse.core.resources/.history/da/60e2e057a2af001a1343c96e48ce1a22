...
<%
	...
	//	웹 애플리케이션의 핵심 공용 기능을 단순한 태그로 캡슐화한 웹 애플리케이션 프로그램 개발 환경
	//1. http://search.maven.org
	//		jstl-1.2.jar
	//		jstl-api-1.2.2.jar
	//2. WEB-INF/lib import
	//3. jsp 파일 내 태그라이브러리 디렉티브 선언(jsp 파일 개별 선언)
	
	IMemberService service = MemberServiceImpl.getInstance();
	List<MemberVO> memberList =  service.memberList();
	pageContext.setAttribute("memberList", memberList);
	
	String het = "업무 진입과 숙련과정 : 1. 자신의 업무 파악 및 업무 준비, 2. 업무 숙련도 향상, 3. 자신의 업무와 연계된 동료들의 업무 파악,"
			     + "4. 비효율적인 업무 환경 효율화하는데 기여";
...
<!--  
	...
	forEach : begin - 반복문의 초기값
			  ...
			  varStatus - 반복문의 상태정보
			  			  ...
			  			  ${stat.first} - 최초 반복문 실행시 true 반환
			  			  ${stat.last} - 최종 반복문 실행시 true 반환
			  			  ${stat.count} - 반복횟수
			  step - 증가치	  
 -->
...
<!-- 
	...
	scope : page(default) 생략시 default pageScope
			...
			application
 -->
<c:set var="jumsu" value="83" scope="page"></c:set>
${jumsu }<br/>
<c:out value="${jumsu }" default="value 속성값이 zero lang. String 또는 null일때 대체 출력 값"></c:out><br/>
<c:if test="${jumsu >= 90 }">수<br/></c:if>
<c:if test="${jumsu < 90 && jumsu >= 80 }">우<br/></c:if>
<c:if test="${jumsu < 80 && jumsu >= 70 }">미<br/></c:if>
...
<c:choose>
	...
	<c:when test="${jumsu < 90 && jumsu >= 80 }">우</c:when>
	<c:when test="${jumsu < 80 && jumsu >= 70 }">미</c:when>
	<c:otherwise>
		분발합시다.
	</c:otherwise>
</c:choose>
...
<h3>jsp:include 액션 태그 대체. 컨텐스트 루트|패스 생략한 서블릿 패스를 설정</h3>
<c:import url="/01/gugudanResult.jsp" var="gugudanResult">
	<c:param name="dan" value="6"></c:param>
</c:import>
${gugudanResult }<br/>
<h3>url 표현</h3>
<!-- 
...
<c:url var="gugudanURI" value="/01/gugudanResult.jsp">
	<c:param name="dan" value="3"></c:param>
</c:url>
