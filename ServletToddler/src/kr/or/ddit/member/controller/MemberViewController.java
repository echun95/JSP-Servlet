...
public class MemberViewController extends HttpServlet{
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		// SerlvetToddler/member/memberView?mem_id=a001
			...
			out.println("$(function(){");
				...
				out.println("$('form').submit(function(){");
					...
					out.println("if(!mem_hometel.validationHOMETEL()){");
						out.println("alert('집 전화번호를 바르게 입력해주세요');");
						out.println("return false;");
					out.println("}");
		...
		// input tag disable='disalbe' 선언되면 값 출력 가능. Form의 태그 Submit시(서버 요청 간) 서버 대상 전송 배제
		// input tag type =hidden 선언되면 값 미출력, Form 태그 Submit시 서버 대상 전송됨. 
