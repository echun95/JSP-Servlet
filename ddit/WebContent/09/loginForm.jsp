...
<!-- 
	...
	해쉬 알고리즘(단방향성 FingerPrint[지문] 활용)
	   - 해싱(암호화)된 암호문은 서버단에서 평문으로 복호화 하지 않고 그 자체를 활용
	   - 적용된 해당 해시 알고리즘이 해커에 뚫리면 해시 알고리즘이 적용된 모든 암호문이 평문화 가능성이 높음
	      * 단어사전입력공격(무차별, 무작위 글자조합 적용), LookUpTable(준비된 해쉬값을 활용 - 뽐뿌 해킹에 활용)
	   - 해시 알고리즘 + Salt(소금치기) : 동일한 비밀번호를 사용하는 유저들의 해쉬값이 동일할 수 있는 보안 취약 상황에서 반복적인 해싱처리와 
	   							 무작위 해싱구성(랜덤한 salt 활용)으로 암호화하고, salt값을 재차 암호화(기타 암호화 알고리즘 활용)
       - SHA1|2|16|32|256|512
       - SHA256(256bit-32byte) or SHA512(512bit-64byte) : hex 표현
